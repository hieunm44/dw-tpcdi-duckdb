...
def customermgmt_convert(filepath):
    with open(filepath+'/CustomerMgmt.xml') as fd:
        doc = xmltodict.parse(fd.read()) 
        fd.close()

    with open(filepath+"/CustomerData.json", "w") as outfile:
        outfile.write(json.dumps(doc))
        outfile.close()

    f = open(filepath+'/CustomerData.json','r')
    ...
    cust_df.to_csv(filepath+'/CustomerMgmt.csv', index = False)

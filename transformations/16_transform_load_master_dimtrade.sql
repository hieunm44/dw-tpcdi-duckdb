...
insert into master.dimtrade
	with trades as (
		...
		, case
			...
			then CAST(EXTRACT(YEAR FROM th.th_dts) * 10000 + EXTRACT(MONTH FROM th.th_dts) * 100 + EXTRACT(DAY FROM th.th_dts) AS NUMERIC)
			else null
		  end as sk_createdateid
		, case
			when (th.th_st_id = 'SBMT' and t.t_tt_id in ('TMB', 'TMS')) or th.th_st_id = 'PNDG'
			then CAST(EXTRACT(HOUR FROM th.th_dts) * 10000 + EXTRACT(MINUTE FROM th.th_dts) * 100 + EXTRACT(SECOND FROM th.th_dts) AS NUMERIC)
			else null

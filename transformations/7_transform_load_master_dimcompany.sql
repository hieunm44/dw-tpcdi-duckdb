...
insert into master.dimcompany
	...
	country, 
	description, 
	CAST(SUBSTRING(foundingdate, 1, 4) || '-' || SUBSTRING(foundingdate, 5, 2) || '-' || SUBSTRING(foundingdate, 7, 2) AS DATE),
	...
	1 as batchid,
	CAST(SUBSTRING(left(f.pts, 8), 1, 4) || '-' || SUBSTRING(left(f.pts, 8), 5, 2) || '-' || SUBSTRING(left(f.pts, 8), 7, 2) AS DATE) as effectivedate,
